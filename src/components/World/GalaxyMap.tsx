...
export const GalaxyMap: React.FC<GalaxyMapProps> = ({ onPointClick }) => {
  ...
  const handleDrag = useCallback(
    (event: any, info: any) => {
      ...
      // deltaX > 0 = moving right, deltaX < 0 = moving left
      // deltaY > 0 = moving down, deltaY < 0 = moving up
      // Invert deltaY since screen coordinates are inverted
  ...
  const handleDragEnd = () => {
    setIsDragging(false);
    // Reset ship to neutral position (0°) when stopped
